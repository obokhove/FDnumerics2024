...
J_values = [200]  # Number of spatial points
...
theta = 1
# Coefficients
epsilon_values = [0.0001, 0.00001, 0.000001] # Diffusion constant
a = 1  # Advection speed
...
for J in J_values:
    ...
    for epsilon in epsilon_values:
        ...
        time_points = [0, int(1*num_steps / 10), int(2*num_steps / 10), int(3*num_steps / 10), int(4*num_steps / 10), int(5*num_steps / 10),int(6*num_steps / 10),int(7*num_steps / 10),int(8*num_steps / 10), int(9*num_steps / 10), num_steps]
        ...
        plt.title(f'Time Profiles for Δx = {Delta_x}, ε = {epsilon}')
        plt.xlabel('x')
        plt.ylabel('u(x,t)')
        plt.grid(True)
        plt.legend()
        plt.savefig(f'8cdx{Delta_x}e{epsilon}.png', dpi=300)
        plt.show()
        # Zoom in near x = -1
        plt.figure(figsize=(10, 6))
        for i, step in enumerate(time_points):
            plt.plot(x, solutions[step], label=f't = {step * Delta_t:.2f}s')
        plt.xlim([-1, -0.98])  # Zoom in near x = -1
        plt.ylim([0, 1.2])
        ...
        plt.grid(True)
        plt.savefig(f'8ddx{Delta_x}e{epsilon}.png', dpi=300)
        plt.show()