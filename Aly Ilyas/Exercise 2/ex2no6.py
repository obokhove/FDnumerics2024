...
theta = 0.9
F1_int = (theta * H0 * u2_('+') + (1 - theta) * H0 * u2_('-'))
F2_int = (theta * g * u1_('+') + (1 - theta) * g * u1_('-'))

F1_l = theta * H0 * u2_ + (1 - theta) * (-H0 * u2_)
F1_r = theta * (-H0 * u2_) + (1 - theta) * H0 * u2_

#F2_l = g * u1_
#F2_r = g * u1_
F2_l = theta * g * u1_ + (1 - theta) * g * u1_
F2_r = theta * g * u1_ + (1 - theta) * g * u1_


#F1_int = theta*u2('+') + (1-theta)*u2('-')
#F1_l = theta*(u2) + (1-theta)*(-u2)
#F1_r = theta*(-u2) + (1-theta)*(u2)
#F2_int = (1-theta)*g*u1('+') + theta*g*u1('-')
#F2_l = (1-theta)*g*u1 + theta*g*u1 # extrapolating condition left solid wall for standing waves
#F2_r = (1-theta)*g*u1 + theta*g*u1 # right
...
while t < Tend-0.5*dt:
	...
	#Et = 0.5 * assemble(Hx * u2_**2 * dx) + 0.5 * assemble(g * u1_**2 * dx)
	energy.append(Et)
