# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 22:07:53 2024
...
def q5(Lp,L, a, J, T, theta, epsilon, CFL, N):
    ...
    while time <= T:
        ...
        if time>a_timestep[counter]:
            ...
            plt.title(f'dx={dx}, epsilon = {epsilon}')
            plt.xlabel('x')
            plt.ylabel('U')
            plt.xlim(-1.0, -0.60)
            plt.legend()
            plt.savefig(f"gulana_q8_epsilon_{epsilon}_dx={dx}.jpeg")
            counter+=1
        
        
q5(-1,1,1,100,1,1,0.000001,1, 200000)
