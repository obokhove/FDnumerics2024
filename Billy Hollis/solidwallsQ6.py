...
if Nbc==1:
    ...
    Fetafluxbcl = - thetaa*Hx*(n[0] * (u20) + (thetaa - 1)*Hx*(n[0] * (u20))) # specified on ghost side
    Fetafluxbcr = -  thetaa*Hx*(n[0] * (u20) + (thetaa- 1)*Hx*(n[0] * (u20))) # extrapolation outflow
else:
    Fetafluxbcl = thetaa*Hx*(n[0] * (uve0)) - (1-thetaa)*Hx*(n[0] * (uve0)) # inner product outcome a scalar fails
    Fetafluxbcr = thetaa*Hx*(n[0] * (uve0)) - (1-thetaa)*Hx*(n[0] * (uve0)) # inner product outcome a scalar fails
...
if Nbc==1:
    ...
    Fuvefluxbcl = (thetaa - 1)*grav*u10 - thetaa*grav*u10 # specified on left ghost side
    Fuvefluxbcr = (thetaa - 1)*grav*u10 - thetaa*grav*u10 # extrapolating condition right bc
else:
