...
if Nbc==1:
    ...
    # Zero flux at left boundary
    #Fetafluxbcl = Constant(0.0)
    # Zero flux at right boundary
    #Fetafluxbcr = Constant(0.0)
...
#etarhs = (deta_test * eta0 * dx
          #- dt * Fetaflux*(deta_test('+') - deta_test('-'))*dS
          #+ dt * Fetafluxbcl * deta_test*ds(1)
         #- dt * Fetafluxbcr * deta_test*ds(2))
...
if Nbc==1:
    ...
    #Fuvefluxbcl = Constant(0.0)
    #Fuvefluxbcr = Constant(0.0)
uverhs = duve_test * uve0 * dx  - dt*Fuveflux*(n[0]*(duve_test('+')-duve_test('-')))*dS
#uverhs = (duve_test * uve0 * dx
      #    - dt * Fuveflux*(n[0]*(duve_test('+') - duve_test('-')))*dS
       #   + dt * Fuvefluxbcl*(n[0]*duve_test)*ds(1)
        #  - dt * Fuvefluxbcr*(n[0]*duve_test)*ds(2))
...
while t <= Tend:
    ...
    if t>tmease+smallfac:
        ...
        ax2.plot(xvals, phi12) #, color[int(i-1)], label = f' $\phi_n: t = {t:.3f}$')
        ax1.legend(loc='upper left', bbox_to_anchor=(1, 1), ncol=5)
...
if Nbc == 1:
 ...
 # define final-time exact solution for the standing wave
    eta_exact_final = Function(DG0).interpolate(
        ck*cos(mint*np.pi*x/Ld)*cos(omeg*t)
    )
    u_exact_final = Function(DG0).interpolate(
        (omeg*ck/H0)*(Ld/(mint*np.pi))*sin(mint*np.pi*x/Ld)*sin(omeg*t)
    )


    # 2) Evaluate exact solution on xvals
    eta_exact_array = np.array([eta_exact_final.at(xi) for xi in xvals])
    u_exact_array   = np.array([u_exact_final.at(xi)   for xi in xvals])

    # compute errors
    error_eta = Function(DG0).interpolate(eta0 - eta_exact_final)
    error_u   = Function(DG0).interpolate(uve0 - u_exact_final)

    L2_eta = sqrt(assemble(error_eta**2 * dx))
    L2_u   = sqrt(assemble(error_u**2   * dx))
    
    ax1.plot(xvals, (H0 + eta_exact_array), 'r--', label='Exact $\eta$')
    ax2.plot(xvals, u_exact_array, 'r--', label='Exact $u$')


    print(f"\n******** Convergence check at final time t={t:.4f} ********")
    print(f"L2 error in eta = {L2_eta:1.6e}")
    print(f"L2 error in u   = {L2_u:1.6e}")
    print("*******************************************************")
...
# Plot topography as a thin black dashed line, no legend:
ax1.plot(Xce, (H0 - Hxt), 'k--', lw=1, label='_nolegend_')
# Possibly also a red dashed line at y=H0 if you want:
ax1.axhline(y=H0, color='r', linestyle='--', lw=1, label='_nolegend_')
# Plot exact solution at final time only
ax1.plot(xvals, H0+eta_exact_array, 'r-', lw=2, label='Exact $\eta$')
ax2.plot(xvals, u_exact_array,       'r-', lw=2, label='Exact $u$')

# Put legend in a nice location
ax1.legend(loc='upper left', bbox_to_anchor=(1,1))
ax2.legend(loc='upper left', bbox_to_anchor=(1,1))

plt.tight_layout()  # often helps spacing
plt.show()
