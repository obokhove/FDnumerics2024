...
#There are 2 methods, 1. Write the weak formulation manually by multiplying the poissons equation with a trial function
#                      2. Minimise the energy funtional to obtain the weak form.

# Use given B.Cs in solver to solve.
...
mesh = UnitSquareMesh(nx,ny,quadrilateral=True) # Defining mesh V and coordinates (x,y).
...
V = FunctionSpace(mesh, 'CG', 3) # Taken Continuous Galerkin 3, which means p=3
# Piecewise linear continuous Galerkin function space or polynomials
...
u_e = Function(V).interpolate(sin(pi*x)*cos(pi*y))
...
outfile = VTKFile('output_128*128_p3.pvd')
outfile.write(u_1, u_2, u_e)

f.interpolate(2*pi**2*sin(pi*x)*cos(pi*y))
