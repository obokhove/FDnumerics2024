...
#
#-----------------------------------------------------#
#Step 1: Write down RG principle and derive the weak formulation
#In Firedrake, this is done by first defining a regular mesh V and spatial coordinates x,y upon this mesh
#Then we can either use method 1 or 2 to obtain u_h which are both well commented
# In Method 1: we write down the weak formulation directly using a trial function
#In Method 2: we write down the energy functional and perform a functional derivative to obtain the weak formulation directly
#For both methods, we then use the solver to solve this by using the provided boundary conditions 
#-----------------------------------------------------------------------------------------------------------------------------#

from firedrake import *

nx = ny = 80 # Try various mesh resolutions, starting coarse, say 16x16 etc.
...
nCG=2
V = FunctionSpace(mesh, 'CG', nCG) # Piecewise linear continuous Galerkin function space or polynomials
...
u_e=Function(V,name='u_e').interpolate(sin(pi*x)*cos(pi*y))
