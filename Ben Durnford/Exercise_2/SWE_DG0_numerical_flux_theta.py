...
if Nbc==1: # standing wave
    m_int = 4
    grav = 1   #grav strength
    H0 = 1.0   #height
    Lx = 2.0*np.pi  #oscillatory distance
    Ld = Lx          #length of the domain
    kw = np.pi*m_int/Ld  #wavenumber
    omeg = grav*kw       #angular frequency
    c_k = 0.2             #magnitude of the oscillation
    N_per = 4             #number of half periods
    Tend = N_per*2.0*np.pi/omeg    #end time of simulation
    print('Tend',Tend)
    dt = Tend/1000               #splits simulation into timesteps
    nmea = 11                    #tracks progression of simulation
    tmeas = [0,Tend/10,2*Tend/10,3*Tend/10,4*Tend/10,5*Tend/10,6*Tend/10,7*Tend/10,8*Tend/10,9*Tend/10,Tend]
    print(f'the length of tmeas is {len(tmeas)}')
    tmease = Tend/10
    dtmeas = Tend/10
    Nx = 500
    #nmea = 21
...
if Nbc==1: # standing wave
    ...
    omeg = grav*kw
    c_k = 0.2
    Nper = 10.0  #number of periods
    eta0 = Function(DG0).interpolate( c_k*cos(m_int*np.pi*x/Ld)*cos(omeg*tijd) ) # u1 is eta
    uve0 = Function(DG0).interpolate( (omeg*c_k/H0)*(Ld/(m_int*np.pi))*sin(m_int*pi*x/Ld)*sin(omeg*tijd) ) # u2 is velocity
    vve0 = 0.0*x   #vertical velocity
...
fig, (ax1, ax2) = plt.subplots(2,dpi=1200)
tsize = 14
...
Xce = np.zeros(Nkel+1)  #array of 41 zeroes
...
colors = cmap(np.linspace(0, 1, len(tmeas)+1))
ax1.plot(xvals,H0+eta12,label=f'{round(t,2)}',color=colors[0])
ax1.plot(Xce,(H0-Hxt),'k',lw=3)
ax1.plot(Xce, np.array([H0 for i in range(len(Xce))]),'--r',lw=2 ) # asymptotics 
ax2.plot(xvals,phi12,color=colors[0])

'''
ax1.plot(Ld*Xce,H0s*(H0-Hxt),'k',lw=3) #, color[int(i-1)],label = f' $\eta_n: t = {t:.3f}$')
ax1.plot( Ld*Xce,H0s*( H0+Ad*(H0/Hxt)**0.25 ),'--r',lw=2 ) # asymptotics 
...
if Nbc==1:
    Fetafluxbcl = thetaa*Hx*(n[0] * (uve0)) - (1-thetaa)*Hx*(n[0] * (uve0)) # left boundary condition for eta
    Fetafluxbcr = thetaa*Hx*(n[0] * (uve0)) - (1-thetaa)*Hx*(n[0] * (uve0)) # right boundary condition for eta
...
E_fig,E_ax=plt.subplots(1,dpi=1200)
...
while t <= Tend:
    ...
    if t>tmease:
        ...
        eta12 = np.array([eta0.at(x) for x in xvals]) # eta12 = np.array([eta0.at(x) for x in xvals])
        phi12 = np.array([uve0.at(x) for x in xvals])
        ax1.plot(xvals,H0+eta12,label=f'{round(t,2)}',color=colors[nnm])
        ax2.plot(xvals,phi12,color=colors[nnm])
    ...
    if t>tmE+smallfac:
        ...
        if Nbc==1:
            # print('energy',t, abs((E-E0)/E0))
            time_list.append(t)
            energy_list.append(abs(E-E0)/E0)
...
#EXACT SOLUTIONS
#etat = Function(DG0).interpolate( c_k*cos(m_int*np.pi*x/Ld)*cos(omeg*Tend) ) # u1 is eta
#uvet = Function(DG0).interpolate( (omeg*c_k/H0)*(Ld/(m_int*np.pi))*sin(m_int*pi*x/Ld)*sin(omeg*Tend) ) # u2 is velocity

etat=(c_k)*np.cos(m_int*np.pi*xvals/Ld)*np.cos(omeg*t) 
uvet=(omeg*c_k/H0)*(Ld/(m_int*np.pi))*np.sin(m_int*np.pi*xvals/Ld)*np.sin(omeg*t)

toc = time.time() - tic
print('Elapsed time (min):', toc/60)
#ax1.plot(xvals,eta12)
#ax2.plot(xvals,phi12)
fig.legend(frameon=False,title='Time',bbox_to_anchor=(0.98, 0.8))
fig.subplots_adjust(right=0.8)
fig.savefig('num_flux_standing_wave.png')

fig2, (ax3, ax4) = plt.subplots(2,dpi=1200)
ax3.set_ylabel(r'$\eta(x,t)$ ',fontsize=tsize)
ax3.grid()
ax4.grid()
ax4.set_xlabel(r'$x$ ',fontsize=tsize)
ax4.set_ylabel(r'$u(x,t)$ ',fontsize=tsize)
ax3.plot(xvals,H0+eta12)
ax4.plot(xvals,phi12,label='Numerical')
ax3.plot(xvals,H0+etat,'--r')
ax4.plot(xvals,uvet,'--r',label='Exact')
fig2.legend(frameon=False,title='Scheme',bbox_to_anchor=(0.98, 0.6))
fig2.subplots_adjust(right=0.75)

#print(len(time_list))
#print(len(energy_list))
E_ax.plot(np.array(time_list),np.array(energy_list)*100,'x',color='darkred',markersize=4)
E_ax.plot(np.array(time_list),np.array(energy_list)*100,'-',color='darkred',alpha=0.4,linewidth=1)
E_ax.set(xlabel='Time, t', ylabel='Scaled energy, $E_s$ / $10^{-2}$')
E_fig.savefig('energy.png')


fig2.savefig('fig2_600_cells.png')
plt.show() 
print('*************** PROGRAM ENDS ******************')
