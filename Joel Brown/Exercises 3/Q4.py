...
import numpy as np
nx = ny = 16 # Mesh resolutions, h=1/nx
p = 4 # Order of Contiuous Galerkin method used
...
u_e = Function(V).interpolate(sin(pi*x)*cos(pi*y)) #Exact solution
...
difference = Function(V, name='|u_h-u_e|') #Defining difference between numerical and exact solutions, for plotting purposes.
difference.dat.data[:] = abs(u_1.dat.data - u_e.dat.data)
...
outfile.write(u_1, u_2, difference)
