...
def get_legendre(k, J, seed=None):
    l = 0
    b_k = []
...
def a_t(U : np.ndarray, j :int, del_t: float, del_x: float, a_val : float, theta : float) -> float:
    ...
    mu_x = del_t / del_x
    return  mu_x*theta*(max(a_val,0)*(U[j+1]-U[j]))
...
def main():
    ...
    del_x = (R - L) / J
    del_t = 0.000025
    a_val = 1.0         #Advection strength set to values of a(t)
    epsilon = 0.0001         #Diffusion strength
    theta = 1.0
    target_time = 1     #length of time to run for in "real time", seconds
    use_legendre = False
    #seed = None
    seed = [0.6149655,0.40498927,0.93575839]
    zoom=False

    T = target_time / del_t
    print(f'T= {T}')
    print(f'mu = {del_t/(del_x**2)}')
    print(f'CFL = {a_val*del_t/(del_x)}')

    x = np.linspace(-1,1,J)
    if use_legendre:
        l, b_k = get_legendre(3, J, seed)
        U = ((1-x)**4)*(1+x)*l
    else:
        U = ((1-x)**4)*(1+x)

    snapshots = [0, 0.001, 0.005, 0.01, 0.1, 0.25, 0.5, 1.0]
    r_head = copy(U)
    ...
    for r,s in list(zip(results, snapshots)):
        ...
        plt.ylabel("U")
        plt.title(f"$CFL (2 d.p)$={round(a_val*del_t/(del_x),2)}, $\epsilon$ = {epsilon}, J={J}")
