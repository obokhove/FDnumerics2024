...
IC = "b"
Alter_flux = False # True (alternating flux) / False (Godunov flux)
...
while t < Tend-0.5*dt:
	...
	if step == output_step:
		...
		ax1.scatter(x_k, u1t_k, marker='x',  color='k',label=fr'$\eta_{{ex}}(x,t={t:.3f})$')
		ax2.scatter(x_k, u2t_k, marker='x', color='k', label=fr'$H_0 u_{{ex}}(x,t={t:.3f})$')
		...
		print('u1')
		e1 = abs(u1.dat.data - u1t_k[0:-1])

		with open(f'error1_{Tend}_alt_{Alter_flux}_cf{CFL}_nx{Nx}.txt', 'w') as f:
		...
		print('u2')
		e2 = abs(u2.dat.data - u2t_k[0:-1])

		with open(f'error2_{Tend}_alt_{Alter_flux}_cf{CFL}_nx{Nx}.txt', 'w') as f:
			for i in range(len(e2)):
				f.write(str(e2[i]))
				f.write('\n')
			f.write(str(e2.mean()))

ax1.set_title(r'DG0 Finite Element Solution of the Shallow Water Equations')
ax2.set_xlabel(r'$x$')
...
ax1.legend()
ax2.legend()
plt.savefig(f'Soln_tend_{Tend}_alt_{Alter_flux}.png')
...
ax.set_ylabel(r'$E(t)$')
ax.grid()
plt.savefig(f'E_tend_{Tend}_alt_{Alter_flux}.png')
#plt.show()
